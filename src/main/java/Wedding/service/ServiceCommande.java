...
public class ServiceCommande implements IServiceCommande {
    ...
    public static List<Pair<ServiceItem, Double>> getServicesReservesDansPanier(int commandeId) throws SQLException {
        List<Pair<ServiceItem, Double>> servicesReserves = new ArrayList<>();

        // Requête SQL pour récupérer les services réservés pour une commande spécifique
        String query = "SELECT s.id, s.nom, s.description, s.prix, s.image_url, s.utilisateur, r.prix_total " +
                "FROM reservation r " +
        ...
        try (Connection connection = MyDatabase.getInstance().getConnection();
            ...
            // Définir l'ID de la commande dans la requête
            ...
            while (resultSet.next()) {
                // Créer un objet ServiceItem à partir des données de la base de données
                ...
                // Récupérer le prix total du service
                double prixTotal = resultSet.getDouble("prix_total");

                // Ajouter la paire (ServiceItem, prixTotal) à la liste
                servicesReserves.add(new Pair<>(service, prixTotal));
            }
